#!/bin/bash
# =============================================================================
# CM4 Lab 01 - Memory swap setup for Ubuntu Server 22.04
# =============================================================================
# The CM4 ships with 1-8 GB of RAM. apt installs of ROS 2 are fine without
...
#
# Run this ON THE CM4 (over SSH is fine). No reboot needed.
# =============================================================================
...
set -u
SWAPFILE=/swapfile
ZRAM_SCRIPT=/usr/local/bin/cm4-zram.sh
ZRAM_UNIT=/etc/systemd/system/cm4-zram.service
...
show_status() {
  ...
  if [ "$TOTAL_MB" -ge "$RECOMMENDED_MB" ]; then
    echo -e "\033[1;32mThat is enough for building ROS 2 packages on the CM4.\033[0m"
  else
...
if [ "${1:-}" = "--uninstall" ]; then
  say "Removing zram"
  systemctl disable --now cm4-zram.service 2>/dev/null
...
say "This CM4 has $((RAM_KB/1024)) MB RAM and $(nproc) cores"
...
#!/bin/bash
# CM4 - bring up zram swap. Installed by setup_swap.sh.
...
ZUNIT
    ...
    systemctl daemon-reload
    systemctl enable cm4-zram.service >/dev/null 2>&1
    systemctl restart cm4-zram.service
    sleep 2
    ZSIZE=$(awk '/zram/{print int($3/1024)}' /proc/swaps)
    [ -n "$ZSIZE" ] && ok "zram active: ${ZSIZE} MB" \
                    || warn "zram did not come up - check: systemctl status cm4-zram"
  else
