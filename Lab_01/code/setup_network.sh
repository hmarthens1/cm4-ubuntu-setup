#!/bin/bash
# =============================================================================
# CM4 Lab 01 - Network setup for Ubuntu Server 22.04 (netplan)
# =============================================================================
...
#
#   eth0   static IP (for the laptop <-> CM4 cable) or DHCP (for a router)
...
#
# Run this ON THE CM4. If you are connected over SSH and the IP of that link
# changes, your session drops - reconnect to the new address.
# =============================================================================

# ----------------------------- SETTINGS --------------------------------------
ETH_MODE="static"                 # "static" or "dhcp"
# Use the row that matches YOUR LAPTOP:
#   Windows (ICS)   : 192.168.137.12/24  gateway 192.168.137.1
#   macOS / Linux   : 192.168.0.12/24    gateway 192.168.0.1
ETH_ADDRESS="192.168.137.12/24"
...
OUT_FILE=$NETPLAN_DIR/01-cm4-network.yaml
...
if [ "$WIFI_MODE" = "ap" ]; then
  ...
  ip link show wlan0 >/dev/null 2>&1 || die "No wlan0 - this CM4 has no Wi-Fi (a 'CM4xxx000' model?)"
fi
...
if [ "$WIFI_MODE" = "ap" ] && ! command -v nmcli >/dev/null; then
  ...
  apt-get update -q && apt-get install -y network-manager dnsmasq-base \
    || die "Could not install network-manager. The CM4 needs internet for this step."
  ok "installed"
fi
